...
class ZATCASimplifiedTaxInvoice
{
    ...
    {
        $populated_template = require ROOT_PATH . '/templates/invoice_billing_reference_template.php';
...
public function getCertifccicateSignature(string $cer): string
{
    ...
    if (!openssl_x509_export($res, $out, false)) {
        throw new \RuntimeException('Failed to export certificate.');
    }

    // Split the output to get the signature part
    $parts = explode('Signature Algorithm:', $out);
    if (count($parts) < 3) {
        throw new \RuntimeException('Could not find Signature Algorithm in the certificate.');
    }

    // Further process the output to extract the signature
    $signature = explode('-----BEGIN CERTIFICATE-----', $parts[2]);
    if (count($signature) < 2) {
        throw new \RuntimeException('Could not find the certificate body.');
    }
    ...
    // Combine the necessary lines and clean up
    $out = implode('', array_slice($signatureLines, 1, 4)); // Get lines 2 to 5
    $out = str_replace([':', ' '], '', $out);
  ...
  return  bin2hex($string);
    ...
    {
        $populated_template = require ROOT_PATH . '/templates/ubl_signature_signed_properties_for_signing_template.php';
    ...
    {
        $populated_template = require ROOT_PATH . '/templates/ubl_signature_signed_properties_template.php';
    ...
    public function defaultUBLExtensions(string $invoice_hash, string $signed_properties_hash, string $digital_signature, string $cleanUpCertificateString, string $ubl_signature_signed_properties_xml_string): string
    {
        ...
        $populated_template = require ROOT_PATH . '/templates/ubl_signature.php';
...
//        }
         ...
         * set invoice lines
         */
        $tax_total_template = require ROOT_PATH . '/templates/tax_total_template.php';
         ...
         */
        $legal_monetary_total_template = require ROOT_PATH . '/templates/legal_monetary_total_template.php';
         ...
         * set invoice lines
         */
        $invoice_line_template = require_once ROOT_PATH . '/templates/invoice_line_template.php';
